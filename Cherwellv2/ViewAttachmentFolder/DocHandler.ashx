...
public class Handler : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        string id = context.Request.QueryString["RecId"].ToString();
        string sConn = System.Configuration.ConfigurationManager.ConnectionStrings["CherwellConnectionString"].ToString();
        SqlConnection objConn = new SqlConnection(sConn);
        objConn.Open();
        //string sTSQL = "select * from Documents where DocumentID=@DocumentID AND InactiveDate IS NuLL";
        ...
        objCmd.Parameters.AddWithValue("@RecId", id);
        ...
        if (file.Rows.Count > 0)
        {
            ...
            string name = (string)row["FilePath"].ToString().Split ('\\').Last();
            string mimeType = MimeMapping.GetMimeMapping((string)row["FileExt"]);
            string mimeType2 = GetMimeType((string)row["FileExt"]);
            string contentType = mimeType2;
            Byte[] data = (Byte[])row["FileContents"];
