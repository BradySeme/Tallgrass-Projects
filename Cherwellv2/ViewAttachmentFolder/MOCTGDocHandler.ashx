...
public class Handler : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        string id = context.Request.QueryString["MOCID"].ToString();
        string sConn = System.Configuration.ConfigurationManager.ConnectionStrings["MOCTallgrassConnectionString"].ToString();
        SqlConnection objConn = new SqlConnection(sConn);
        objConn.Open();
        string sTSQL = "SELECT [documentfilename], [documentblob], [documentuploadpath], [documentfilesize], [documentextension], [fkmocid] FROM [MOCTallgrass].[dbo].[documents] WHERE fkmocid=@MOCID";
        //string sTSQL = "SELECT [RecID],[AttachType],[AttachFlags],[FilePath],[FileContents],[FileExt] FROM [Cherwell].[dbo].[TrebuchetAttach] WHERE RecId=@RecId";
        SqlCommand objCmd = new SqlCommand(sTSQL, objConn);
        objCmd.CommandType = CommandType.Text;
        objCmd.Parameters.AddWithValue("@MOCID", id);
        ...
        if (file.Rows.Count > 0)
        {
            ...
            string name = (string)row["documentfilename"];
            string mimeType = MimeMapping.GetMimeMapping((string)row["documentextension"]);
            string mimeType2 = GetMimeType((string)row["documentextension"]);
            string contentType = mimeType2;
            Byte[] data = (Byte[])row["documentblob"];
            //int FileSize = Convert.ToInt32(row["documentfilesize"].ToString());
